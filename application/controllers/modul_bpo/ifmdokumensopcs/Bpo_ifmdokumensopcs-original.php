		...
		if (isset($_POST['submit'])) {
            ...
            if (!$this->upload->do_upload("txtfile")){
				...
				if ($lccekdokumensop['file_name'] != ' ') {
                    $txtfile = $lccekdokumensop['file_name'];
				} else {
					$txtfile = ' ';
				}
                //echo "success upload [txtfile]";
            }
    
            // Upload File Form
            $config["upload_path"] 		= $upload_folder . "form/";
			$config['allowed_types'] = 'gif|jpg|png|zip|rar|pdf|doc|docx|ppt|pptx|xls|xlsx|txt';
			$config['max_size'] = '30000'; // kb
			//$config['encrypt_name'] = TRUE;
            $config["file_name"]		= $txtfile2;
            ...
            if (!$this->upload->do_upload("txtfile2")) {
                ...
                $lccekdokumenform = $this->upload->data();

				if ($lccekdokumenform['file_name'] != ' ') {
					$txtfile2 = $lccekdokumenform['file_name'];
				} else {
					$txtfile2 = ' ';	
				}
                                
                //echo "success upload [txtfile2]";
            }
