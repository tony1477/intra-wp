  ...
  echo "<div class='col-xs-12'>
          <div class='box box-info box-solid' style='margin: -20px -5px 15px 1px;'>
            <div class='box-header'>
              <h3 class='box-title'><span class='glyphicon glyphicon-new-window'></span> Insert Data Departemen</h3>
            </div>

            <div class='box-body'>";
              $attributes = array('class'=>'form-horizontal','role'=>'form','onsubmit'=>'return ifcekemptytext()','action'=>'<?php echo base_url()');
              echo form_open_multipart('modul_tbl/ifmdepartemencs/tbl_ifmdepartemencs/ifinsertrecord', $attributes);
              echo "<div class='col-md-12'>
                <table class='table table-condensed table-bordered'>
                    <tbody>
                        <input type='hidden' name='id' value=''>
                        <tr><th scope='row'>Kode Departemen</th><td><input type='text' id='txtkode' width='130px' maxlength='3' class='form-control' name='txtkode' onkeyup='this.value = this.value.toUpperCase()' autofocus></td></tr>
                        <tr><th scope='row'>Nama Departemen (IDN)</th><td><textarea id='txtnama' class='form-control' name='txtnama'></textarea></td></tr>
                        <tr><th scope='row'>Nama Departemen (ENG)</th><td><textarea id='txtnama2' class='form-control' name='txtnama2'></textarea></td></tr>
                        <tr><th scope='row'>Kode Divisi</th><td><select id='cmbgroup' class='js-example-basic-single form-control' name='cmbgroup' style='height: 32px; width: 250px;' required>
          ...
          <div class='box-footer'>
              ...
              <a href='".base_url().$this->uri->segment(1)."/ifmdepartemencs/tbl_ifmdepartemencs/ifshowtbldepartemen'><button type='button' class='btn btn-primary pull-right' data-toggle='tooltip' data-placement='bottom' title='Kembali'><i class='glyphicon glyphicon-log-out'></i> Tutup</button></a>
          </div>
        </div></form>";
?>
...
<script type="text/javascript">
    ...
    if (!$("#txtkode").val()) {
			  alert('Mohon mengisi kode departemen');
			  $("#txtkode").focus()
			  return false;
		}

		if (!$("#txtnama").val()) {
			  alert('Mohon mengisi nama departemen');
			  $("#txtnama").focus()
			  return false;
		}

    if (lcGroup < 1) {
      alert('Mohon mmemilih nama divisi');
